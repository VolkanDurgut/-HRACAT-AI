      ...
      <div className="w-full md:w-[500px] bg-white flex flex-col justify-center px-8 md:px-14 py-12 shadow-2xl z-20 overflow-y-auto">
        ...
        {isForgotPassword ? (
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-300">
            <h2 className="text-2xl font-bold text-slate-900 mb-2">Ücretsiz Başlayın</h2>
            <p className="text-sm text-slate-500 mb-6">Google hesabınızla saniyeler içinde 14 günlük ücretsiz denemenizi başlatın.</p>
            {authError && <div className="mb-4 p-3 rounded-md bg-red-50 border border-red-100 text-sm text-red-600 font-medium">{authError}</div>}

            <button type="button" onClick={handleGoogleLogin} disabled={isProcessing} className="w-full flex items-center justify-center gap-2 bg-white border border-slate-300 text-slate-700 text-sm font-medium py-2.5 px-4 rounded-md hover:bg-slate-50 transition-colors">
              {isProcessing ? <Loader2 size={18} className="animate-spin" /> : (
                <>
                  ...
                  Google ile 14 Gün Ücretsiz Başla
                </>
              )}
            </button>
