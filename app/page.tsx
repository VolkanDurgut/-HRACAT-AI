...
const AuthScreen = ({ 
  ...
  const handleGoogleLogin = async () => {
    setAuthError(null);
    setIsProcessing(true);
    showToast("İşleminiz başlatılıyor, yönlendiriliyorsunuz...", "success");
    setTimeout(async () => {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: `${window.location.origin}/dashboard` }
      });
      if (error) { setAuthError("Google işlemi başlatılamadı."); setIsProcessing(false); }
    }, 1500);
  };
        ...
        {isForgotPassword ? (
          ...
          <div className="animate-in fade-in slide-in-from-left-4 duration-300">
            ...
            <p className="text-sm text-slate-500 mb-6">
              {authType === "login" ? "Hesabınıza erişmek için giriş yapın." : "Kredi kartı ile 14 günlük ücretsiz denemenizi başlatın."}
            </p>
            {authError && <div className="mb-4 p-3 rounded-md bg-red-50 border border-red-100 text-sm text-red-600 font-medium">{authError}</div>}
            
            <button type="button" onClick={handleGoogleLogin} disabled={isProcessing} className="w-full flex items-center justify-center gap-2 bg-white border border-slate-300 text-slate-700 text-sm font-medium py-2.5 px-4 rounded-md hover:bg-slate-50 transition-colors mb-6">
              ...
              Google ile {authType === "register" ? "14 Gün Ücretsiz Başla" : "Devam Et"}
            </button>

            <div className="relative my-6">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div>
              <div className="relative flex justify-center text-sm"><span className="px-2 bg-white text-slate-400 text-xs">veya</span></div>
            </div>

            <form onSubmit={authType === "login" ? handleEmailLogin : handleRegister} className="space-y-4">
              <div>
              ...
              <button type="submit" disabled={isProcessing || !email || !password} className="w-full mt-2 py-2.5 rounded-md text-white font-medium text-sm transition-colors hover:bg-emerald-600 bg-emerald-500 flex justify-center">
                {isProcessing ? <Loader2 size={18} className="animate-spin" /> : (authType === "login" ? "Giriş Yap" : "Kredi Kartı ile Başla")}
              </button>
            </form>

            <div className="mt-6 text-center text-sm text-slate-600">
              {authType === "login" ? (
                <>Hesabınız yok mu? <button type="button" onClick={() => setAuthType("register")} className="font-medium text-emerald-600 hover:underline">Kayıt Olun</button></>
              ) : (
                <>Zaten hesabınız var mı? <button type="button" onClick={() => setAuthType("login")} className="font-medium text-emerald-600 hover:underline">Giriş Yapın</button></>
              )}
            </div>
          </div>
        )}
    ...
    <div className="min-h-screen flex flex-col w-full bg-white font-sans text-slate-900 selection:bg-emerald-200">
      ...
      <nav className="fixed top-0 w-full h-16 bg-white/80 backdrop-blur-md border-b border-slate-200 z-50 flex items-center justify-between px-6 lg:px-12">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => goToAuth("register")} className="text-sm font-medium bg-emerald-500 text-white px-4 py-1.5 rounded-md hover:bg-emerald-600 transition-colors shadow-sm">
            Projenizi başlatın
          </button>
        </div>
      </nav>
      ...
      <section className="py-24 bg-[#1C1C1C] text-center px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center justify-center">
            <button onClick={() => goToAuth("register")} className="bg-emerald-500 text-white px-8 py-3 rounded-md font-medium text-base hover:bg-emerald-600 transition-all shadow-sm">
              Projenizi başlatın
            </button>
          </div>
        </div>
